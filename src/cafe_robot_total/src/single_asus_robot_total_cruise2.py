...
class cafe_robot_total():
 ...
 def single_amcl(self):
  subprocess.Popen('roslaunch machine asus_robot_amcl_without_map.launch',shell=True)
 
 # map server
 def map_server(self):
  subprocess.Popen('rosrun planner ReadMap.py',shell=True)
 ...
 def robot_controller(self):
  subprocess.Popen('roslaunch machine robot_controller_cruise2.launch',shell=True)
